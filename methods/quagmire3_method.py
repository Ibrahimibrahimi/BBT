...
class Quagmire3Method(BaseMethod):
    name = "Quagmire III"
    description = "Quagmire III: autokey variant substitution cipher"
    category = "Cipher"
    ...
    def encode(self, text: str) -> str:
        ...
        key_stream = list(indicator.upper())
        result = []
        for ch in letters:
            rot = ord(key_stream[-1]) - ord('A')
            sub = shifted[rot:] + shifted[:rot]
            sub_map = dict(zip(alphabet, sub))
            enc = sub_map.get(ch, ch)
            result.append(enc)
            key_stream.append(enc)
